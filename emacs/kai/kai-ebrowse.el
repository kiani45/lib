...
(defcustom ebrowse-default-declaration-column 40
...
(defcustom ebrowse-default-column-width 40
...
(defun ebrowse-choose-from-browser-buffers ()
  ...
  (let* ((buffers (ebrowse-known-class-trees-buffer-list)))
    ...
    ;; Kai: using default BROWSE file path
    ...
    (if (string-match "\\/datapower\\/" (pwd))
        (progn 
          (unless (file-exists-p (concat (getenv "WORKDIR") "/datapower/BROWSE")) (error "No BROWSE file found"))
          (find-file-noselect (concat (getenv "WORKDIR") "/datapower/BROWSE")))
      (unless (file-exists-p "./BROWSE") (error "No BROWSE file found"))
...
\\{ebrowse-member-mode-map}"
  ...
  (setq mode-name "Ebrowse-Members"
	...
	buffer-read-only t
    ;; Kai: display member in long form
	;;ebrowse--long-display-flag nil
    ebrowse--long-display-flag t
...
(defun ebrowse-display-toggle-static-functions-member-list ()
  "Toggle display of static members."
  (interactive)
  (setf ebrowse--static-functions-display-flag (not ebrowse--static-functions-display-flag))
  (if ebrowse--static-functions-display-flag
      (ebrowse-display-static-functions-member-list)
    (ebrowse-display-functions-member-list)))
  ...
  (let* ((classes ebrowse--tree-obarray)
	...
	(ebrowse-member-mode))
      ...
      (setq ebrowse--member-list (funcall list class)
        ...
        ;; Kai: alway stand-alnoe
	    ;; ebrowse--frozen-flag stand-alone
        ebrowse--frozen-flag t
...
It is a list (TREE ACCESSOR MEMBER)."
  (let ((buffer (get-buffer ebrowse-member-buffer-name)))
    (cond ((null buffer)
       ...
       ;; Kai: show stand-alone buffer
	   ;; (switch-to-buffer (ebrowse-display-member-buffer (second info) nil (first info))))
       (switch-to-buffer (ebrowse-display-member-buffer (second info) t (first info))))
	  (t
